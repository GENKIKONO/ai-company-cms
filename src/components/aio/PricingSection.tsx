...
const getColorClasses = (color: string, popular: boolean = false) => {
  const colors = {
    blue: {
      bg: popular ? 'bg-[var(--aio-primary)]' : 'bg-white',
            ...
            return (
              <div key={plan.name} className="relative">
                ...
                <div className={`relative ${colors.bg} ui-card p-8 transition-all duration-300 h-full card-min-height-lg ${plan.popular ? '' : 'border-2 ' + colors.border}`}>
                  ...
                  <Link
                    href="/organizations"
                    className={`block w-full text-center px-6 py-4 ${colors.button} rounded-xl font-semibold transition-all duration-300 ui-flat`}
                  >
                    {plan.buttonText}
                  </Link>
                </div>
