 ...
 * P4-4: Embedding ジョブ状況の簡易表示ウィジェット
 */
...
import type { EmbeddingMetrics } from '@/lib/embedding-client';

interface EmbeddingMetricsResponse {
  success: boolean;
  data: EmbeddingMetrics;
  error?: string;
}

export default function EmbeddingStatusWidget() {
  ...
  const fetchMetrics = async () => {
    try {
      const response = await fetch('/api/admin/embeddings/metrics');
      const result: EmbeddingMetricsResponse = await response.json();
  ...
  if (!metrics) {
    return (
      <div className="bg-white rounded-lg shadow p-4">
        <h3 className="text-sm font-medium text-gray-900 mb-2">Embeddingシステム状況</h3>
        <p className="text-sm text-gray-500">データを取得できませんでした</p>
      </div>
    );
  }

  const hasActiveJobs = metrics.pending_jobs + metrics.processing_jobs > 0;

  return (
    <div className="bg-white rounded-lg shadow">
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-gray-900">Embeddingシステム状況</h3>
          <Link
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500">Embedding数</div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          {metrics.processing_jobs > 0 && (
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <CubeIcon className="w-4 h-4 text-[var(--aio-info)] mr-2" />
                <span className="text-sm text-gray-700">処理中</span>
              </div>
              <span className="text-sm font-medium text-[var(--aio-info)]">
                {metrics.processing_jobs.toLocaleString()}
              </span>
            </div>
          )}
        ...
        <div className="mt-4 pt-3 border-t border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-sm font-medium ${
              metrics.success_rate_percent >= 95 ? 'text-green-600' :
              metrics.success_rate_percent >= 80 ? 'text-yellow-600' : 'text-red-600'
            }`}>
        ...
        {hasActiveJobs && (
          <div className="mt-3 p-2 bg-[var(--aio-info-surface)] rounded-lg">
            <p className="text-xs text-[var(--aio-info)]">
              Embedding処理が実行中です。バッチ処理で順次実行されます。
            </p>
          </div>
        )}
        ...
        {/* 使用モデル情報 */}
        {Object.keys(metrics.embeddings_by_model).length > 0 && (
          <div className="mt-3">
            <div className="text-xs text-gray-500 mb-1">使用モデル</div>
            <div className="space-y-1">
              {Object.entries(metrics.embeddings_by_model)
                .sort(([,a], [,b]) => b - a)
                .slice(0, 2)
                .map(([model, count]) => (
