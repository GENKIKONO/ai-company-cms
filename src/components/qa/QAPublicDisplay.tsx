...
export default function QAPublicDisplay({ organizationSlug, className = '' }: QAPublicDisplayProps) {
  ...
  const fetchQAData = useCallback(async () => {
    try {
      ...
      logger.error('Error fetching Q&A data', { data: error instanceof Error ? error : new Error(String(error)) });
    } finally {
      setLoading(false);
    }
  ...
  const logQAView = async (qnaId: string) => {
    try {
      ...
      if (hasAlreadyViewed) {
        logger.debug(`Q&A ${qnaId} already viewed in this session, skipping duplicate log`);
        return;
      }
      ...
      logger.warn('Failed to log Q&A view', { data: error });
    }
  };
