...
export default function OrganizationForm({ initialData, onSubmit, isLoading = false }: Props) {
  ...
  const validateForm = async () => {
    ...
    try {
      ...
      Object.entries(requiredFields).forEach(([field, label]) => {
        if (!formData[field as keyof OrganizationFormData] || 
            String(formData[field as keyof OrganizationFormData]).trim() === '') {
  ...
  const handleApplyCandidates = (candidates: ExtractedCandidates) => {
    ...
    // エラーをクリア
    const fieldsToUpdate = Object.keys(candidates) as Array<keyof OrganizationFormData>;
    setErrors(prev => {
