...
### 8. お問い合わせ
個人情報に関するお問い合わせ：privacy@aiohub.ai
...
export default function ConsentManager({
  ...
  const loadExistingConsents = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Failed to load consents', { data: error instanceof Error ? error : new Error(String(error)) });
    } finally {
      setLoading(false);
    }
  ...
  const recordConsent = async (type: string, granted: boolean) => {
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to record consent', { data: error instanceof Error ? error : new Error(String(error)) });
      }
    } catch (error) {
      logger.error('Failed to record consent', { data: error instanceof Error ? error : new Error(String(error)) });
    }
