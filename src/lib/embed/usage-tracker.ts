...
export class EmbedUsageTracker {
  ...
  async track(usage: EmbedUsage): Promise<void> {
    try {
      ...
      logger.error('Usage tracking failed', { data: error instanceof Error ? error : new Error(String(error)) });
  ...
  async getDailyStats(
    ...
    try {
      ...
      logger.error('Failed to fetch daily stats', { data: error instanceof Error ? error : new Error(String(error)) });
      return [];
    }
  }
  ...
  async getMonthlyStats(
    ...
    try {
      ...
      logger.error('Failed to fetch monthly stats', { data: error instanceof Error ? error : new Error(String(error)) });
      return null;
    }
  }
  ...
  async checkMonthlyLimit(
    ...
    try {
      ...
      logger.error('Failed to check monthly limit', { data: error instanceof Error ? error : new Error(String(error)) });
      // エラー時は制限なしとして扱う（安全側）
      return {
        allowed: true,
        currentViews: 0,
  ...
  async getTopSources(
    ...
    try {
      ...
      logger.error('Failed to fetch top sources', { data: error instanceof Error ? error : new Error(String(error)) });
      return [];
    }
  }
  ...
  async getRealTimeStats(organizationId: string): Promise<{
    ...
    try {
      ...
      logger.error('Failed to fetch realtime stats', { data: error instanceof Error ? error : new Error(String(error)) });
      return {
        activeWidgets: 0,
        todayViews: 0,
        todayClicks: 0,
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to flush usage batch', { data: error instanceof Error ? error : new Error(String(error)) });
        // 失敗したデータを戻す（再試行用）
        this.batchQueue.unshift(...batch);
      }
    } catch (error) {
      logger.error('Batch flush error', { data: error instanceof Error ? error : new Error(String(error)) });
    }
