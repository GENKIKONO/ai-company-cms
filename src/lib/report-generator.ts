...
export async function collectMonthlyData(
  ...
  try {
    ...
    if (orgError || !org) {
      logger.error('Organization not found for report generation', { organizationId, orgError });
      return null;
    }
    ...
    logger.error('Failed to collect monthly data', error instanceof Error ? error : new Error(String(error)));
    return null;
  }
}
...
export async function saveReportToStorage(
  ...
  try {
    ...
    logger.error('Error saving report to storage', error instanceof Error ? error : new Error(String(error)));
    return null;
  }
}