...
async function queryNominatim(address: string): Promise<GeocodeResult | null> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      logger.error(`Geocoding attempt ${attempt + 1} failed:`, { data: error });
...
export async function geocodeJP(address: string): Promise<GeocodeResult> {
  ...
  for (const variation of variations) {
    try {
      ...
      logger.warn(`Failed to geocode variation "${variation}":`, { data: error });
  ...
  if (prefectureCityMatch) {
    try {
      ...
      if (result) {
        logger.warn('Only prefecture/city level geocoding succeeded for', { data: address });
        return {
          ...result,
      ...
      logger.warn('Prefecture/city fallback also failed', { data: error });
    }
  }
