  ...
  try {
    ...
    checks.forEach((check, index) => {
      if (check.status === 'fulfilled') {
        ...
        logger.error(`Compliance check ${index} failed:`, { data: check.reason });
      }
    });
    ...
    logger.error('Compliance check error', { data: error instanceof Error ? error : new Error(String(error)) });
...
export async function reportComplianceViolation(
  ...
  try {
    ...
    logger.error('Compliance violation report error', { data: error instanceof Error ? error : new Error(String(error)) });
    return false;
  }
}