...
export async function startTrial(organizationId: string): Promise<boolean> {
  try {
    ...
    logger.error('Failed to start trial', { data: error instanceof Error ? error : new Error(String(error)) });
    return false;
  }
}
...
export async function transitionToStarter(organizationId: string): Promise<boolean> {
  try {
    ...
    logger.error('Failed to transition to starter', { data: error instanceof Error ? error : new Error(String(error)) });
    return false;
  }
}
...
export async function checkAndTransitionExpiredTrials(): Promise<number> {
  try {
    ...
    if (error || !expiredTrials) {
      logger.error('Failed to fetch expired trials', { data: error instanceof Error ? error : new Error(String(error)) });
      return 0;
    }
    ...
    logger.error('Failed to check expired trials', { data: error instanceof Error ? error : new Error(String(error)) });
    return 0;
  }
}
