...
export async function getUserPlan(): Promise<UserPlanInfo> {
  try {
    ...
    if (orgError) {
      logger.error('[getUserPlan] Error fetching organization:', { data: orgError });
      return { plan: 'trial', isActive: false };
    }

    if (!organization) {
      logger.debug('[getUserPlan] No organization found for user');
      return { plan: 'trial', isActive: false };
    }
    ...
    logger.error('[getUserPlan] Unexpected error', { data: error instanceof Error ? error : new Error(String(error)) });
    return { plan: 'trial', isActive: false };
  }
}
