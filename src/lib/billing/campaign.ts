...
export async function fetchActiveCheckoutLink(
  ...
  try {
    ...
    logger.error('Failed to fetch active checkout link', error instanceof Error ? error : new Error(String(error)));
    return null;
  }
}
...
export async function getPrivateCheckoutLink(
  ...
  try {
    ...
    logger.error('Failed to fetch private checkout link', error instanceof Error ? error : new Error(String(error)));
    return null;
  }
}