...
export class SitemapGenerator {
  ...
  async generateOrganizationsSitemap(): Promise<SitemapUrl[]> {
    try {
      ...
      if (error) {
        logger.error('Failed to fetch organizations for sitemap', { data: error instanceof Error ? error : new Error(String(error)) });
        return [];
      }
      ...
      logger.error('Error generating organizations sitemap', { data: error instanceof Error ? error : new Error(String(error)) });
      return [];
    }
  }
