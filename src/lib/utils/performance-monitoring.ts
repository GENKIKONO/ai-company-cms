...
export class WebVitalsMonitor {
  ...
  private observeLCP() {
    ...
    try {
      ...
      logger.warn('LCP observation failed', { data: error });
    }
  }
  ...
  private observeFID() {
    ...
    try {
      ...
      logger.warn('FID observation failed', { data: error });
    }
  }
  ...
  private observeCLS() {
    ...
    try {
      ...
      logger.warn('CLS observation failed', { data: error });
    }
  }
  ...
  private observeFCP() {
    ...
    try {
      ...
      logger.warn('FCP observation failed', { data: error });
    }
  }
  ...
  private notifyCallbacks() {
    this.callbacks.forEach(callback => {
      try {
        ...
        logger.error('Performance callback error', { data: error instanceof Error ? error : new Error(String(error)) });
      }
    });
  }
...
export class ResourceMonitor {
  ...
  private initializeResourceObserver() {
    ...
    try {
      ...
      logger.warn('Resource observation failed', { data: error });
    }
  }
  ...
  public startMemoryMonitoring(interval = 30000): () => void {
    const intervalId = setInterval(() => {
      ...
      if (usage && usage.percentage > 80) {
        logger.warn('High memory usage detected', { data: usage });
      }
    }, interval);
  ...
  public sendToAnalytics(report?: ReturnType<PerformanceMonitor['getFullReport']>) {
    ...
    if (typeof window !== 'undefined' && window.plausible) {
      try {
        ...
        logger.error('Failed to send performance data to Plausible', { data: error instanceof Error ? error : new Error(String(error)) });
      }
    }
