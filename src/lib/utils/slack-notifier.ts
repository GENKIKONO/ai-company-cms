...
export class SlackNotifier {
  ...
  async sendMessage(message: SlackMessage): Promise<boolean> {
    if (!this.enabled || !this.webhookUrl) {
      logger.debug('[Slack] Notification disabled or webhook URL not configured');
      return false;
    }
    ...
    try {
      ...
      logger.error('[Slack] Failed to send notification', { data: error instanceof Error ? error : new Error(String(error)) });
      return false;
    }
  }
