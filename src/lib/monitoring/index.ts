...
export class ConsoleLogTransport implements LogTransport {
  async send(entry: LogEntry): Promise<void> {
    ...
    switch (entry.level) {
      case LogLevel.ERROR:
        utilLogger.error(message, { data: entry.metadata });
        if (entry.stack) utilLogger.error(entry.stack);
        break;
      case LogLevel.WARN:
        utilLogger.warn(message, { data: entry.metadata });
        break;
      case LogLevel.INFO:
        utilLogger.info(message, { data: entry.metadata });
        break;
      case LogLevel.DEBUG:
        utilLogger.debug(message, entry.metadata);
        break;
    }
  }
}
...
export class PerformanceMonitor {
  ...
  collectMetrics(): void {
    ...
    this.logger.info('Performance metrics collected', { data: { metrics } });
  }
  ...
  recordWebVitals(vitals: WebVitalsData): void {
    this.logger.info('Web Vitals recorded', { data: { vitals } });
    
    // しきい値チェック
    if (this.isPerformanceIssue(vitals)) {
      this.logger.warn('Performance issue detected', { data: { vitals } });
    }
  }
...
export function initMonitoring(config?: {
  ...
  logger.info('Monitoring system initialized', { data: { config: !!config } });
}