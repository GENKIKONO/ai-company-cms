...
export async function checkOpsAdmin(): Promise<OpsGuardResult> {
  try {
    ...
    if (!isAdmin(user.email)) {
      logger.error('[checkOpsAdmin] Not admin user:', { data: user.email });
      return {
        isAuthorized: false,
        user,
        reason: 'NOT_ADMIN'
    ...
    if (!opsAdminCookie || opsAdminCookie.value !== '1') {
      logger.error('[checkOpsAdmin] Missing ops_admin cookie for admin:', { data: user.email });
      return {
        isAuthorized: false,
        user,
    ...
    logger.error('[checkOpsAdmin] Unexpected error', { data: error instanceof Error ? error : new Error(String(error)) });
    return {
      isAuthorized: false,
...
export async function requireOpsAdminPage(): Promise<void> {
  ...
  if (!result.isAuthorized) {
    logger.error('[requireOpsAdminPage] Access denied:', { data: result.reason });
    redirect('/ops/login');
  }
}
...
export async function getOpsAdminStatus() {
  try {
    ...
    logger.error('[getOpsAdminStatus] Unexpected error', { data: error instanceof Error ? error : new Error(String(error)) });
    return {
