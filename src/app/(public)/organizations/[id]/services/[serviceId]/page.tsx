...
import type { AppUser, Organization, Service } from '@/types/legacy/database';
...
export default function EditServicePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('Failed to update service', { data: error instanceof Error ? error : new Error(String(error)) });
      setErrors({ submit: 'サービス情報の更新に失敗しました' });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async () => {
    try {
      ...
      logger.error('Failed to delete service', { data: error instanceof Error ? error : new Error(String(error)) });
