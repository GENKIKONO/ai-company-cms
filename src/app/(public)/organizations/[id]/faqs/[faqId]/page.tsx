...
export default function EditFAQPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [orgResult, faqResult, categoriesResult] = await Promise.all([
        ...
        if (faqResult.data) {
          ...
          setFormData({
            ...
            sort_order: faqData.sort_order || faqData.order_index || 1
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('Failed to update FAQ', { data: error instanceof Error ? error : new Error(String(error)) });
      setErrors({ submit: 'FAQの更新に失敗しました' });
    } finally {
      setSubmitting(false);
    }
