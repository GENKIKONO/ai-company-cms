...
export default function NewFAQPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await createFAQ(organizationId, formData);
      
      if (result.data) {
        router.push(`/organizations/${organizationId}/faqs/${result.data.id}`);
      } else {
      ...
      <div>
        ...
        <nav className="flex mb-8" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-4">
            ...
            <li>
              <span className="text-gray-900 font-medium">新しいFAQを追加</span>
            </li>
          </ol>
        </nav>
        ...
        {/* ページタイトル */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">新しいFAQを追加</h1>
          <p className="text-lg text-gray-600">
            {organization.name}のよくある質問を追加してください
          </p>
        </div>
            ...
            <div className="border border-gray-200 rounded-lg">
              <details className="group">
                ...
                <div className="px-4 pb-4">
                  ...
                  {formData.category && (
                    <div className="mt-2">
                      <span className="inline-block px-2 py-1 bg-[var(--aio-info-muted)] text-[var(--aio-info)] text-xs rounded">
                        {formData.category}
                      </span>
                    </div>
                  )}
