...
export default function HearingRequestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('Hearing request submission error', { data: error instanceof Error ? error : new Error(String(error)) });
  ...
  return (
    <div>
      <div>
        ...
        {/* 注意事項 */}
        <div className="mt-8 bg-[var(--aio-info-surface)] border border-[var(--aio-info-border)] rounded-lg p-6">
          <h3 className="font-medium text-[var(--aio-info)] mb-2">ご利用について</h3>
          <ul className="text-sm text-[var(--aio-info)] space-y-1">
