...
export default function NewCaseStudyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('Failed to create case study', { data: error instanceof Error ? error : new Error(String(error)) });
      setErrors({ submit: 'ケーススタディの作成に失敗しました' });
    } finally {
      setSubmitting(false);
    }
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formData.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-[var(--aio-info-muted)] text-[var(--aio-info)]"
                      >
                        {tag}
                        <button
                          type="button"
                          onClick={() => removeTag(tag)}
                          className="ml-2 text-[var(--aio-primary)] hover:text-[var(--aio-primary)]"
                        >
                          ×
                        </button>
                      </span>
