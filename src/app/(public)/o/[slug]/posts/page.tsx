...
import type { Organization, Post } from '@/types/legacy/database';;
...
async function getPostsData(slug: string): Promise<PostsPageData | null> {
  try {
    ...
    logger.error('Failed to fetch posts data', { data: error instanceof Error ? error : new Error(String(error)) });
    return null;
  }
}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <Link
                  ...
                  className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md hover:border-[var(--aio-info-border)] transition-all"
                >
