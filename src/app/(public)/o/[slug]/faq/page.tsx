...
async function getFAQData(slug: string): Promise<FAQPageData | null> {
  try {
    ...
    logger.error('Failed to fetch FAQ data', { data: error instanceof Error ? error : new Error(String(error)) });
    return null;
  }
}
                      ...
                      <summary className="flex items-start justify-between p-6 cursor-pointer hover:bg-gray-50 transition-colors">
                        <div className="flex-1 pr-4">
                          <div className="flex items-start">
                            <span className="bg-[var(--aio-info-muted)] text-[var(--aio-info)] px-2 py-1 text-xs font-medium rounded-full mr-3 mt-1 flex-shrink-0">
                              Q{index + 1}
                            </span>
