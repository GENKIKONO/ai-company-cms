...
async function getServiceData(slug: string, serviceId: string): Promise<ServiceDetailData | null> {
  try {
    ...
    logger.error('Failed to fetch service data', { data: error instanceof Error ? error : new Error(String(error)) });
    return null;
  }
}
      ...
      <div className="min-h-screen bg-gray-50">
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <article className="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                {service.image_url ? (
                  ...
                  <div className="w-full h-64 md:h-72 bg-gray-100 rounded-md shadow-sm flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">{service.name}</h3>
                      <p className="text-sm text-gray-500">サービスイメージ</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {service.video_url && (
              <div className="px-6 py-8 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">サービス紹介動画</h2>
                {service.video_url.includes('youtube.com') || service.video_url.includes('youtu.be') ? (
                  <div className="relative aspect-video max-w-2xl mx-auto rounded-lg overflow-hidden">
                    <iframe
                      src={service.video_url.replace('watch?v=', 'embed/').replace('youtu.be/', 'youtube.com/embed/')}
                      title={`${service.name}の紹介動画`}
          ...
          {/* お問い合わせ・連絡先 */}
          <div className="mt-8 bg-[var(--aio-info-surface)] rounded-lg p-6">
