...
export default function AdminReviewsPage() {
  ...
  const fetchPendingReviews = async () => {
    try {
      ...
      logger.error('Error fetching reviews', { data: error instanceof Error ? error : new Error(String(error)) });
      toast({
        title: "エラー",
  ...
  const handleReviewAction = async (orgId: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      logger.error('Error performing review action', { data: error instanceof Error ? error : new Error(String(error)) });
      toast({
        title: "エラー",
