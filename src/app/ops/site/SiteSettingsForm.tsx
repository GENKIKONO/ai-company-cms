...
export default function SiteSettingsForm({ initialData }: SiteSettingsFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('Site settings save error', { data: error instanceof Error ? error : new Error(String(error)) });
      addToast({
        type: 'error',
        title: '保存エラー',
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      logger.error('Site settings reset error', { data: error instanceof Error ? error : new Error(String(error)) });
      addToast({
        type: 'error',
        title: 'リセットエラー',
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[var(--aio-primary)] focus:border-[var(--aio-primary)]"
      ...
      <div className="flex justify-between pt-4">
        ...
        <div className="space-x-3">
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-[var(--aio-primary)] hover:bg-[var(--aio-primary-hover)] text-white rounded-md shadow-sm text-sm font-medium disabled:opacity-50"
            disabled={loading}
          >
            {loading ? '保存中...' : '保存'}
          </button>
