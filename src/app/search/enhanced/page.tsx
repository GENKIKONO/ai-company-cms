...
function EnhancedSearchForm() {
  ...
  const performSearch = useCallback(async (searchFilters: AdvancedFilters, pageNum = 1) => {
    ...
    try {
      ...
      logger.error('Search failed', { data: error instanceof Error ? error : new Error(String(error)) });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      ...
      logger.error('Suggestions fetch failed', { data: error instanceof Error ? error : new Error(String(error)) });
      setSuggestions([]);
    }
  }, []);
