...
export default function ContactsPage() {
  ...
  const fetchContacts = async () => {
    try {
      ...
      logger.error('Error fetching contacts', { data: error instanceof Error ? error : new Error(String(error)) });
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateContactStatus = async (contactId: string, status: Contact['status']) => {
    try {
      ...
      logger.error('Error updating contact status', { data: error instanceof Error ? error : new Error(String(error)) });
    }
  };
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      logger.error('Error sending reply', { data: error instanceof Error ? error : new Error(String(error)) });
    } finally {
      setReplying(false);
    }
  };
                ...
                {selectedContact.status !== 'archived' && (
                  <div className="border-t pt-6">
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--aio-primary)]"
