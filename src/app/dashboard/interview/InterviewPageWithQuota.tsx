  ...
  try {
    ...
    if (user?.id) {
      ...
      if (organization?.data?.id) {
        ...
        const quotaData = await fetchOrgQuotaUsage(organization.data.id, 'ai_interview');
        aiInterviewQuota = normalizeQuotaForProps(quotaData);
      }
    }
  } catch (error) {
    // fail-open: エラー時はquotaをnullのままにして、Client Componentは従来通り動作
    console.error('Failed to fetch ai_interview quota, proceeding without quota display:', error);
  }
