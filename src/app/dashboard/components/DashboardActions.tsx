...
export default function DashboardActions({ organization, context = 'quickActions' }: DashboardActionsProps) {
  ...
  const handleShareLink = async () => {
    ...
    try {
      ...
      toast.className = 'fixed top-4 right-4 bg-[var(--status-success)] text-white px-4 py-2 rounded-md shadow-lg z-50';
      toast.textContent = '共有リンクをコピーしました！';
      document.body.appendChild(toast);
      setTimeout(() => document.body.removeChild(toast), 3000);
    } catch (error) {
      logger.error('Failed to copy to clipboard', { data: error instanceof Error ? error : new Error(String(error)) });
      
      // エラートースト
      const toast = document.createElement('div');
      toast.className = 'fixed top-4 right-4 bg-[var(--status-error)] text-white px-4 py-2 rounded-md shadow-lg z-50';
  ...
  if (context === 'activity') {
    return (
      <div className="mt-6 pt-4 border-t border-[var(--dashboard-card-border)]">
        <button
          onClick={handleShowAllActivity}
          className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] font-medium"
        >
          すべてのアクティビティを表示
        </button>
      </div>
    ...
    <div className="mt-6 pt-6 border-t border-[var(--dashboard-card-border)]">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button
          ...
          className="flex items-center justify-center p-3 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--aio-surface)] rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          ...
          className="flex items-center justify-center p-3 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--aio-surface)] rounded-lg transition-colors"
        >
          <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
