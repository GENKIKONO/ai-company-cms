...
export default function MaterialViewPage() {
  ...
  const fetchMaterial = useCallback(async () => {
    try {
      ...
      logger.error('Failed to fetch material:', { data: err });
  ...
  const logMaterialView = useCallback(async () => {
    try {
      ...
      if (hasAlreadyViewed) {
        logger.debug(`Material ${materialId} already viewed in this session, skipping duplicate log`);
        return;
      }
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        logger.warn('Failed to log download stats', { data: statsError });
      }
      ...
      logger.error('Download failed', { data: error instanceof Error ? error : new Error(String(error)) });
      alert('ダウンロードに失敗しました');
    } finally {
