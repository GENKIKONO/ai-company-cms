...
export default function DocumentList({ organizationId }: DocumentListProps) {
  ...
  const getStatusDisplay = (status: Document['status']) => {
    switch (status) {
      ...
      case 'processing':
        return { text: '処理中', color: 'bg-[var(--aio-warning-muted)] text-[var(--aio-warning)]' };
      case 'ready':
        return { text: '利用可能', color: 'bg-[var(--aio-success-muted)] text-[var(--aio-success)]' };
      case 'error':
        return { text: 'エラー', color: 'bg-[var(--aio-danger-muted)] text-[var(--aio-danger)]' };
      default:
        return { text: '不明', color: 'bg-[var(--aio-surface)] text-[var(--color-text-primary)]' };
    }
  };
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-medium text-[var(--color-text-primary)] mb-4">アップロード済み文書</h3>
        <div className="flex justify-center items-center h-32">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--aio-indigo)]"></div>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-[var(--aio-danger-muted)] border border-[var(--aio-danger)] rounded-md p-4">
          <div className="flex">
            <div className="flex-shrink-0">
              <svg className="h-5 w-5 text-[var(--aio-danger)]" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
              </svg>
            </div>
            <div className="ml-3">
              <h3 className="text-sm font-medium text-[var(--aio-danger)]">エラー</h3>
              <div className="mt-2 text-sm text-[var(--aio-danger)]">
                <p>{error}</p>
              </div>
            </div>
          </div>
    ...
    <div className="bg-white rounded-lg shadow">
      <div className="px-6 py-4 border-b border-[var(--dashboard-card-border)]">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-[var(--color-text-primary)]">アップロード済み文書</h3>
      ...
      {documents.length === 0 ? (
        <div className="p-6 text-center">
          <svg className="mx-auto h-12 w-12 text-[var(--color-icon-muted)] mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          <h3 className="text-lg font-medium text-[var(--color-text-primary)] mb-2">文書がありません</h3>
        ...
        <div className="divide-y divide-[var(--dashboard-card-border)]">
          {documents.map((document, index) => (
            <div key={index} className="p-6 hover:bg-[var(--aio-surface)]">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center">
                    <div className="flex-shrink-0">
                      <svg className="h-8 w-8 text-[var(--aio-danger)]" fill="currentColor" viewBox="0 0 20 20">
