...
export default async function DashboardPage() {
  try {
    logger.debug('[Dashboard] Rendering started');
    logger.debug('[VERIFY][MOBILE_UI] Dashboard with mobile optimizations loading');
    ...
    // 認証状態のエラーまたは未ログインの場合
    if (authError || !user) {
      logger.debug('[Dashboard] No authentication - redirecting to login');
      return (
    ...
    if (!org || !org.id) {
      logger.debug('[Dashboard] Authenticated user but no organization found');
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6">
