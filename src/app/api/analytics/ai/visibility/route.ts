...
export async function GET(request: NextRequest) {
  try {
    ...
    if (scoresError) {
      logger.error('Error fetching visibility scores:', scoresError);
      return NextResponse.json(
    ...
    if (trendError) {
      logger.error('Error fetching trend data:', trendError);
      return NextResponse.json(
    ...
    logger.error('[GET /api/analytics/ai/visibility] Unexpected error:', { errorId, error });
...
export async function HEAD(request: NextRequest) {
  try {
    ...
    logger.error('[HEAD /api/analytics/ai/visibility] Error:', error);
    return new NextResponse(null, { status: 500 });
  }
}