...
export async function GET(request: NextRequest) {
  try {
    ...
    if (aiError) {
      logger.error('Failed to fetch AI scores:', aiError);
      return NextResponse.json(
    ...
    if (seoError) {
      logger.error('Failed to fetch SEO metrics:', seoError);
      return NextResponse.json(
    ...
    if (botError) {
      logger.error('Failed to fetch bot logs:', botError);
    }
    ...
    logger.error('[GET /api/analytics/ai/combined] Unexpected error:', { errorId, error });
