...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (count && count > 0) {
        logger.info('Returning cached GSC data', { collectionId, cachedRecords: count });
    ...
    if (includeQueries && topQueries.length > 0) {
      ...
      if (queryError) {
        logger.error('Failed to save query metrics', { collectionId, error: queryError });
      } else {
    ...
    if (topPages.length > 0) {
      ...
      if (pageError) {
        logger.error('Failed to save page metrics', { collectionId, error: pageError });
      } else {
