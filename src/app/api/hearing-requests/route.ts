...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Hearing request API error', { data: error instanceof Error ? error : new Error(String(error)) });
    return NextResponse.json(
      {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fetchError) {
      logger.error('Database fetch error:', { data: fetchError });
      return NextResponse.json(
        {
          error: 'DATABASE_ERROR',
    ...
    logger.error('Hearing request GET API error', { data: error instanceof Error ? error : new Error(String(error)) });
    return NextResponse.json(
      {
        error: 'INTERNAL_ERROR',
