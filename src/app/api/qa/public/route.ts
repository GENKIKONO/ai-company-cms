...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (error) {
      logger.error('Error fetching public entries', { data: error instanceof Error ? error : new Error(String(error)) });
    ...
    if (catError) {
      logger.error('Error fetching categories:', { data: catError });
    }
