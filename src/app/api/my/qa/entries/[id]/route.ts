/**
 * /api/my/qa/entries/[id] - QAエントリ個別操作（取得・更新・削除）
 * 【認証方式】
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        return applyCookies(NextResponse.json({ error: 'Entry not found' }, { status: 404 }));
      }
      logger.error('Error fetching entry', { data: error instanceof Error ? error : new Error(String(error)) });
      return applyCookies(NextResponse.json({ error: 'Failed to fetch entry' }, { status: 500 }));
    }

    return applyCookies(NextResponse.json({ data: entry }));
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (fetchError || !existingEntry) {
      return applyCookies(NextResponse.json({ error: 'Entry not found or access denied' }, { status: 404 }));
    }
    ...
    const contentString = `${updateData.question}|${updateData.answer}|${(updateData.tags as string[]).join(',')}`;
    ...
    if (error) {
      ...
      return applyCookies(NextResponse.json({ error: 'Failed to update entry' }, { status: 500 }));
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error) {
      ...
      return applyCookies(NextResponse.json({ error: 'Failed to delete entry' }, { status: 500 }));
    }
    ...
    return applyCookies(NextResponse.json({ message: 'Entry deleted successfully' }));
