...
export async function GET(request: NextRequest) {
  try {
    ...
    if (reportsError) {
      logger.error('[Reports API] Failed to fetch reports', reportsError);
    ...
    if (countError) {
      logger.error('[Reports API] Failed to get reports count', countError);
    }
    ...
    logger.error('[Reports API] Unexpected error', error instanceof Error ? error : new Error(String(error)));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (checkError && checkError.code !== 'PGRST116') { // Not "not found" error
      logger.error('[Reports API] Error checking existing report', checkError);
      return NextResponse.json(
    ...
    if (existingReport) {
      ...
      if (updateError) {
        logger.error('[Reports API] Failed to update report status', updateError);
        return NextResponse.json(
      ...
      if (insertError) {
        logger.error('[Reports API] Failed to create report record', insertError);
        return NextResponse.json(
          { error: 'Failed to start report generation' },
          { status: 500 }
        );
    ...
    logger.error('[Reports API] Unexpected error in POST', error instanceof Error ? error : new Error(String(error)));
