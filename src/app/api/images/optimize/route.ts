  ...
  try {
    ...
    if (options) {
      try {
        ...
        logger.error('Options parsing error', { data: error instanceof Error ? error : new Error(String(error)) });
      }
    }
    ...
    logger.error('Image optimization error', { data: error instanceof Error ? error : new Error(String(error)) });
    return NextResponse.json(
      { error: '画像最適化に失敗しました' },
      { status: 500 }
    );
