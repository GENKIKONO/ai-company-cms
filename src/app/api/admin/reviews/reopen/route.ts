...
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      logger.error('[Reviews Reopen] Failed to update organization status', updateError);
      return NextResponse.json(
    ...
    if (auditError) {
      logger.error('[Reviews Reopen] Failed to create audit log', auditError);
    ...
    logger.error('[Reviews Reopen] Unexpected error', error instanceof Error ? error : new Error(String(error)));
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('[Reviews Reopen Check] Unexpected error', error instanceof Error ? error : new Error(String(error)));
