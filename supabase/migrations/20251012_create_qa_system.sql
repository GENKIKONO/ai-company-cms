-- Q&A Knowledge System for AIO Hub
...
CREATE TABLE IF NOT EXISTS public.qa_entries (
  ...
  search_vector tsvector GENERATED ALWAYS AS (
    setweight(to_tsvector('japanese', question), 'A') ||
    setweight(to_tsvector('japanese', answer), 'B') ||
    setweight(to_tsvector('japanese', array_to_string(tags, ' ')), 'C')
  ) STORED
);
