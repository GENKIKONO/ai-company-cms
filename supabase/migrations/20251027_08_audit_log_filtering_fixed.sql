...
-- JSONBデータのフィルタリング関数
CREATE OR REPLACE FUNCTION filter_sensitive_jsonb(input_jsonb JSONB)
RETURNS JSONB AS $$
DECLARE
  filtered_data JSONB;
BEGIN
  ...
  -- JSONBを文字列に変換してフィルタリング後、再度JSONBに変換
  filtered_data := filter_sensitive_data(input_jsonb::TEXT)::JSONB;
...
EXCEPTION
  WHEN OTHERS THEN
    -- JSON解析エラーの場合は安全な形式で返す
    RETURN jsonb_build_object(
      '_error', 'filtering_failed',
      '_original_length', length(input_jsonb::TEXT),
      '_filtered_at', NOW()
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- フィルタリング済み監査ログビュー（修正版）
...
SELECT 
  ...
  risk_level,
  created_at,
  filter_sensitive_jsonb(additional_data) as additional_data
...
BEGIN
  ...
  LOOP
    ...
    WHERE created_at BETWEEN start_date AND end_date
    AND (
      COALESCE(query_text, '') ~ pattern_record.regex_pattern OR
      COALESCE(user_agent, '') ~ pattern_record.regex_pattern OR
      COALESCE(additional_data::TEXT, '') ~ pattern_record.regex_pattern
    );
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    LOOP
      IF COALESCE(NEW.query_text, '') ~ pattern_record.regex_pattern OR
         COALESCE(NEW.user_agent, '') ~ pattern_record.regex_pattern THEN
    ...
    IF has_sensitive_data THEN
      ...
      NEW.additional_data := COALESCE(NEW.additional_data, '{}'::jsonb) || 
        jsonb_build_object(
          ...
          'original_query_length', COALESCE(length(original_query), 0),
          'filtered_at', NOW()
        );
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '=== 監査ログ機密情報フィルタリングシステム実装完了（修正版） ===';
  RAISE NOTICE '1. sensitive_data_patterns テーブルと初期パターン作成';
  RAISE NOTICE '2. filter_sensitive_data() フィルタリング関数';
  RAISE NOTICE '3. filter_sensitive_jsonb() JSONBフィルタリング関数';
  RAISE NOTICE '4. filtered_audit_logs ビュー（機密情報マスキング済み）';
  RAISE NOTICE '5. generate_sensitive_data_report() レポート関数';
  RAISE NOTICE '6. auto_filter_audit_log() 自動フィルタリングトリガー';
  RAISE NOTICE '7. manage_sensitive_pattern() パターン管理関数';
  RAISE NOTICE '8. detect_data_leakage() データ漏洩検出関数';
  RAISE NOTICE '';
  RAISE NOTICE '使用例:';
