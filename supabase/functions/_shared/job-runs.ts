/**
 * job_runs_v2 統一操作ヘルパー
 * EPIC 3-6: ジョブ/バッチの観測性（軽量トランザクション化）
 * 
 * Supabase確定仕様準拠:
 * - UNIQUE(job_name, idempotency_key) 制御
 * - 軽量トランザクション（INSERT→競合時SELECT）
 * - service_role経由のみ書き込み
 * - error_message 2KB制限 + meta.error_details
 */
...
export interface BeginRunResult {
  success: boolean;
  record: JobRunRecord;
  is_duplicate: boolean; // 既存レコード取得時 true
  error?: string;
}
...
function processErrorMessage(
  ...
  if (!errorMessage) {
    return { truncated_message: null, updated_meta };
  }
  ...
  return { truncated_message: truncatedMessage, updated_meta };
}
...
function sanitizeJobMeta(meta: JobMeta | undefined): JobMeta | null {
  ...
  // input_summary/output_summaryは制限付きでコピー
  ...
  // error_details（PII除去済みを前提）
  if (meta.error_details) {
    sanitized.error_details = {
      message_full: meta.error_details.message_full?.slice(0, 5000), // 5KB制限
      cause: meta.error_details.cause?.slice(0, 1000),
      context: meta.error_details.context ? 
        JSON.stringify(meta.error_details.context).slice(0, 2000) : undefined
    };
  }
...
/**
 * ジョブ開始（軽量トランザクション化）
 * idempotency_key による重複制御とUNIQUE違反ハンドリング
 */
export async function beginRun(
  ...
  try {
    ...
    // 1. INSERT running を試行（idempotency_key予約）
    try {
      ...
      if (insertError.code === '23505' && request.idempotency_key) {
        ...
        if (existingRecord.status === 'running' || existingRecord.status === 'pending') {
          ...
          // succeeded/failed/cancelled等の場合は新規実行を推奨
          logger?.warn('Job with same idempotency_key exists in final state', {
            job_id: existingRecord.id,
            job_name: request.job_name,
            existing_status: existingRecord.status,
            suggestion: 'Consider using new idempotency_key for retry'
          });
...
export async function completeSuccess(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('status', 'running') // running状態のレコードのみ更新（競合防止）
      .select()
      .single();
    ...
    if (!data) {
      throw new Error('No running job found to complete (may be already completed or not exist)');
    }
...
export async function completeFailure(
  ...
  try {
    ...
    // error_message の処理（2KB制限 + meta.error_details）
    ...
    const { data, error } = await supabase
      ...
      .update({
        ...
        retry_count: supabase.raw('retry_count + 1'), // アトミックにインクリメント
        meta: sanitizedMeta,
        updated_at: finishedAt
      })
      .eq('id', request.job_id)
...
export async function requestCancel(
  ...
  try {
    const { data, error } = await supabase
      .from('job_runs_v2')
      .update({
        meta: supabase.raw("jsonb_set(meta, '{cancel_requested}', 'true')"),
...
export async function checkConcurrentRunning(
  ...
  try {
    const { count, error } = await supabase
      ...
      .eq('job_name', jobName)
      .eq('status', 'running')
      .gte('started_at', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()); // 24時間以内
    
    if (error) {
      logger?.error('Failed to check concurrent running jobs', { error: error.message });
      return { can_run: true, current_running: 0 }; // エラー時は実行を許可
    }
