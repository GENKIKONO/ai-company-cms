-- ========================================
-- AI Citations Views and RLS Policies
-- ========================================
...
-- Index 2: ai_citations_items response_id + source_id (for JOIN optimization) 
CREATE INDEX IF NOT EXISTS idx_ai_citations_items_response_source
ON ai_citations_items (response_id, source_id)
WHERE source_id IS NOT NULL;

-- Index 3: ai_content_units session_id + organization_id (for integrity checks)
CREATE INDEX IF NOT EXISTS idx_ai_content_units_session_org
ON ai_content_units (session_id, organization_id)
WHERE organization_id IS NOT NULL;
...
-- 3. mv_ai_citations_org_period - 組織×期間集計マテリアライズドビュー
-- ========================================
...
-- 3. v_ai_citations_integrity_daily - 管理者ダッシュボード用整合性指標
-- ========================================
...
-- 5. View RLS (Views inherit table RLS automatically, but explicit grants)
-- ========================================
...
-- SELECT schemaname, tablename, indexname FROM pg_indexes WHERE indexname LIKE 'idx_ai_citations%';
